...
- **Approved next strategy — C15: LR=1e-4 + on-the-fly augmentation**:
  ...
  - **Required [AGENT] changes**: (1) Add `--augment` flag to `Skills/train_cnn.py`; (2) Create `configs/cnn_tess_c15.json` with LR=1e-4, batch=32, patience=20, full-unfreeze from epoch 1.
