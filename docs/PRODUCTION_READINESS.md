...
- **Current data gate**: Combined splits VALIDATED; C13/C14/C15/C16 all rejected; no CNN checkpoint approved for promotion. **T1-1 planning cycle required** — the next attempt must address data (more/better TESS labels) or a materially different architecture strategy. Do not run another incremental LR or regularization experiment.
