...
Last reviewed: 2026-06-20 (C15 rejected — AUC 0.8353, LR tuning exhausted; C16 with batch normalization + stronger L2 next)
...
- **Approved next strategy — C16: batch normalization + stronger L2 regularization**:
  - **Hypothesis**: Overfitting is the bottleneck (C15 train_loss=0.46→0.28 while val_loss=0.77→1.03). C16 adds two regularization changes: `use_batch_norm=true` (implicit regularizer via feature normalization per conv layer) and `weight_decay=1e-2` (10× stronger L2 from 1e-3). Note: BN was rejected as C6 with 22 batches/epoch (running stats too sparse); C16 has 153 batches/epoch (4892 examples / batch 32) — stats converge correctly. `early_stopping_patience` raised from 20 to 25 to give BN models more time to stabilize.
  - **Status**: `configs/cnn_tess_c16.json` committed (LR=1e-4, weight_decay=1e-2, use_batch_norm=true, min_lr=1e-6, lr_scheduler_patience=10, augment=true, batch=32, patience=25, full-unfreeze from epoch 1).
  - **Do not rebuild splits**: `data/tess_combined_cnn_splits/` is validated and reusable for C16.
