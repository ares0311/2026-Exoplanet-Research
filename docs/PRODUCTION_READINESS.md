...
Test baseline: 2,954 default tests passing; 2 `integration_live` tests excluded by
the configured marker expression (2026-07-17; 6×6 gate: 25.2s, 10/10 gates
...
Version note: 0.2.89 is the current patch level. 0.2.8 fixed QLP stitch
...
Version 0.2.64 closes the accompanying fixed-width integration defect: the
live CDS payload omits two optional trailing blanks on unflagged rows, yielding
valid 71-byte records alongside flagged 73-byte records. The parser now pads
...
KICs, 111 exact pinned skips, zero failures/downloads/persisted embeddings, six
temporary arrays removed, and one frozen test opening. The CNN retained a wide
held-out lead (AUC/AP/top-100 0.923096/0.899184/91) over Chronos-Bolt tiny
(0.722778/0.696344/71), Astromer2 (0.708984/0.659679/67), and the statistical
baseline (0.699402/0.607780/67). Version 0.2.76 records the precommitted
`no_external_added_value` outcome. Broad extraction, training, promotion, and
production scoring remain unauthorized. Aggregate SHA-256 is
`3d24363b…4952bd`; seven Run Report commits end at `1200612`.
...
Version 0.2.78 adds `missing_transit_fraction`, the diagnostic named as the
next bounded increment in 0.2.77's roadmap note. It reuses the existing
per-window resolution test (local sideband baseline, twice-noise half-depth
gate) already used for duration/midpoint measurement to count, among
predicted windows with sufficient cadence coverage, the fraction that never
resolved a significant dip — evidence against genuine periodicity distinct
from the pre-existing data-gap-fraction diagnostic (absent coverage, not
unresolved coverage). Wired into `log_score_planet()` (−0.70) and
`log_score_instrumental()` (+0.60); `None` unless at least two windows have
coverage to test. See `docs/SCORING_MODEL.md §23`. Depth/asymmetry and
