...
- **Remaining next `[HUMAN]` action**: continue the resumable Kepler processing batch. `--workers 4` is now live-validated across two consecutive runs; recommended: `--max-targets 250 --workers 4` with `caffeinate -i` (or larger `--max-targets` if the operator wants to move faster). Repeated runs safely continue from the SQLite `done` set regardless of worker count.
