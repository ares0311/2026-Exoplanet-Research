...
- **Required next batch**: After the bounded BLS/vetting fix is merged, run `Skills/star_scanner.py --max-stars 200 --pipeline QLP --exptime long --max-period-grid-points 20000 --workers 4 --request-delay 0.5 --log logs/discovery_run_006_qlp_progress_safe.json`, then rank and filter candidates with `Skills/rank_candidates.py` and `Skills/alert_filter.py`
