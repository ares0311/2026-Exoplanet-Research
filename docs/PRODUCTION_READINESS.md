...
Last reviewed: 2026-06-20 (C14 rejected — AUC 0.8319, LR=3e-5 too conservative; C15 with LR=1e-4 + augmentation next)
...
- **Approved next strategy — C15: LR=1e-4 + on-the-fly augmentation**:
  - **Hypothesis**: LR=1e-4 (10× above C14's too-low 3e-5, 10× below C13's too-high 1e-3) should explore wider parameter space without destabilizing pretrained weights. C14 train_loss=0.48 vs val_loss=0.77 gap confirms overfitting; on-the-fly augmentation (gaussian noise + phase shift) via `--augment` flag in `train_cnn.py` closes the gap without new data.
  - **Status**: `configs/cnn_tess_c15.json` now committed (LR=1e-4, min_lr=1e-6, lr_scheduler_patience=10, augment=true, batch=32, patience=20, full-unfreeze from epoch 1). No code change to `train_cnn.py` needed — augmentation was already enabled (noise_fraction=0.05, scale 0.9–1.1) in all C12/C14 configs. The scheduler decay fix (min_lr=1e-6 vs C14's 1e-7, patience=10 vs 6) prevents the LR from collapsing to near-zero in ~40 epochs.
  - **Do not rebuild splits**: `data/tess_combined_cnn_splits/` is validated and reusable for C15.
- **Current authorized runbook**: `docs/CNN_PRODUCTION_RUNBOOK.md`
- **Current promotion gate**: raw held-out test AUC ≥ 0.85; calibrated held-out test F1 ≥ 0.80; calibration must not worsen held-out test Brier score or ECE
- **Kepler pretraining gate**: **LOCAL PRETRAINED ON MPS** — `checkpoints/cnn_kepler_pretrain/best.pt`, SHA-256 `c782d7af61171b3f58447f7a49343c86618c447292a71bd28d540807835787c7`; Python 3.14.3 venv, PyTorch 2.12.0; startup banner `device=mps`; best epoch 19, best validation loss 0.3905, best validation AUC 0.9186; final epoch 34 val AUC 0.9123; use as pretrained init for C15
- **Current data gate**: Combined splits VALIDATED; C13 and C14 rejected; C15 pending [AGENT] changes + [HUMAN] training; no CNN checkpoint approved for promotion
- **Gate check (C15)**: `.venv/bin/python Skills/evaluate_cnn_checkpoint.py --split-dir data/tess_combined_cnn_splits --checkpoint checkpoints/cnn_tess_c15/best.pt --output-calibration checkpoints/cnn_tess_c15/calibration.json`
