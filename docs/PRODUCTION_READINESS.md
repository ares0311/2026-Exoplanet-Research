...
Test baseline: 2,493 default tests passing, 2 integration_live deselected
...
Version note: 0.2.19 is the current patch level. 0.2.8 fixed QLP stitch
...
instead of a lock-guarded monkeypatch-and-restore.
...
- **First live 2-shard test PASS, self-reported (2026-07-03)**: the human ran 2 concurrent shards (`--shard-count 2 --workers 4` each). Both auto-committed their own run reports: shard 0 processed 250 targets (273 snippets) in 8,541s; shard 1 processed 250 (277 snippets) in 8,573s, 0 failures in either. Combined wall-clock ~8,573s for 500 targets = ~17.1s/target, versus the best single-tab rate of 26.3s/target — a real ~35% combined gain, though short of full 2x scaling. Cumulative: 1,280/6,515 targets done, 5,235 remaining.
...
- **Remaining next `[HUMAN]` action**: re-run the same 2-shard recipe on version 0.2.19+ and compare the per-target rate against the 17.1s/target baseline above; a similar-or-better rate with no new errors validates the fix, a regression or new `ERROR:`/timeout flags mean back off. Recommended: `--max-targets 250 --workers 4 --shard-index {0,1} --shard-count 2` with `caffeinate -i` in two tabs. Repeated runs safely continue from the SQLite `done` set regardless of worker or shard count.
...
| 2,493 default tests, ruff clean, mypy clean | ✅ |
