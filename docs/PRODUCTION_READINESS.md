...
Last reviewed: 2026-06-20 (C13 rejected — AUC 0.8342, LR too high; C14 with low-LR config on combined corpus next)
...
- **Approved next strategy — C14 low-LR combined corpus training**:
  - **Combined splits**: `data/tess_combined_cnn_splits/` VALIDATED (train 4,892 / val 1,049 / test 1,033; validator PASS 2026-06-20).
  - **C14 training plan**: Reuse `data/tess_combined_cnn_splits/` (no rebuild needed); train with `configs/cnn_tess_finetune_c12.json` (LR=3e-5, batch=32, patience=20, full-unfreeze from epoch 1); same Kepler pretrain init. Hypothesis: LR=3e-5 (100× lower than C13's 1e-3) will prevent val_loss spike and allow the model to train stably on 4,892 examples to a higher AUC plateau.
  - **Do not rebuild splits**: `data/tess_combined_cnn_splits/` is validated and reusable for C14.
...
- **Kepler pretraining gate**: **LOCAL PRETRAINED ON MPS** — `checkpoints/cnn_kepler_pretrain/best.pt`, SHA-256 `c782d7af61171b3f58447f7a49343c86618c447292a71bd28d540807835787c7`; Python 3.14.3 venv, PyTorch 2.12.0; startup banner `device=mps`; best epoch 19, best validation loss 0.3905, best validation AUC 0.9186; final epoch 34 val AUC 0.9123; use as pretrained init for C14
- **Current data gate**: Combined splits VALIDATED; C13 rejected; C14 not yet trained; no CNN checkpoint approved for promotion
- **Gate check (C14)**: `.venv/bin/python Skills/evaluate_cnn_checkpoint.py --split-dir data/tess_combined_cnn_splits --checkpoint checkpoints/cnn_tess_c14/best.pt --output-calibration checkpoints/cnn_tess_c14/calibration.json`
