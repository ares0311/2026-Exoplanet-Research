...
Last reviewed: 2026-06-12 (third training run evaluated)
...
- **Root cause (candidates 2 and 3)**: Persistent 9–10 point val→test AUC gap (0.8177→0.7180; 0.8235→0.7283) across both runs with the same seed-42 splits; stronger regularization (v1 config) did not close the gap; the gap is a property of the seed-42 split assigning harder examples to the test set, not a pure overfitting problem
- **Next step**: Rebuild splits with seed=7 and retrain with `configs/cnn_retrain_v1.json`; recipe:
  1. `rm -rf data/cnn_splits/`
  2. `python Skills/build_cnn_training_data.py data/tess_snippets.jsonl --output-dir data/cnn_splits --seed 7`
  3. `python Skills/cnn_split_validator.py data/cnn_splits`
  4. `caffeinate -dims python Skills/train_cnn.py --split-dir data/cnn_splits --checkpoint-dir models/cnn_v2/ --config configs/cnn_retrain_v1.json`
  5. `python Skills/evaluate_cnn_checkpoint.py --split-dir data/cnn_splits --checkpoint models/cnn_v2/best.pt --output-calibration models/cnn_v2/calibration.json`
