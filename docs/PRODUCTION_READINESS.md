...
- **Approved next strategy — Path A first**:
  - **Path A — More labeled TESS data**: Approved on 2026-06-18. Inventory additional ExoFOP TOI/CTOI CP/KP/FP/FA labels absent from `data/tess_snippets_v2.jsonl`, fetch phase-folded snippets only after agent review of the target count and label balance, then build `data/tess_cnn_splits_v3`; target ≥ 5,000 usable training examples before expecting the CNN production gate to close
...
- **Current data gate**: Kepler and TESS split validators passed; `data/tess_cnn_splits` has train/val/test = 1,477 / 318 / 315; the first fine-tuned TESS checkpoint is rejected and no CNN artifact is approved for promotion; Path A v3 inventory/splits are pending
...
|---|---|---|
| Expanded TESS label inventory | Run live ExoFOP TOI/CTOI inventory against `data/tess_snippets_v2.jsonl`, then review whether target count and label balance justify a long MAST fetch | Human + agent |
