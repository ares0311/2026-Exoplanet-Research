...
Last reviewed: 2026-06-30 (JWST A3 merged — PR #141; TESS novelty restructure B1-B4 merged — PR #139; live scanner startup/target-selection fix merged — PR #143; PR #145 worker/ETA fix merged; run006 completed with real QLP scan evidence; run008 targeted follow-up reproduced both filtered candidates under the stitch-normalization and feature-output fixes, but false-positive diagnostics remain review-blocking)
...
Version note: 0.2.8 is the current patch level for the production-blocking QLP
stitch-normalization fix and the `exo --output` feature-serialization fix used
by run008 candidate review.
