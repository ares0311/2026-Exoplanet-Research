...
| `checkpoints/cnn_tess_c18/` | **NOT YET TRAINED** — `configs/cnn_tess_c18.json` committed | Ignored | Planned: train from `data/tess_combined_cnn_splits` with Kepler pretrain; `configs/cnn_tess_c18.json` (LR=1e-4, weight_decay=1e-3, use_batch_norm=false, augment=true, patience=25, freeze_conv_epochs=10); hypothesis: freezing conv for first 10 epochs lets FC head warm up to TESS domain before full unfreeze, avoiding the val_loss explosion seen in C13–C17; train command: `caffeinate -dims .venv/bin/python Skills/train_cnn.py --split-dir data/tess_combined_cnn_splits --checkpoint-dir checkpoints/cnn_tess_c18 --config configs/cnn_tess_c18.json --pretrained-checkpoint checkpoints/cnn_kepler_pretrain/best.pt --device auto` | [HUMAN] train and evaluate with `evaluate_cnn_checkpoint.py`; promote only after evaluator PASS plus explicit human approval |
