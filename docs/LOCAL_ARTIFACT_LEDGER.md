...
| `logs/discovery_run_001.json` | **PENDING** — first real discovery scan not yet run | Ignored | Expected output from the B5 200-target `star_scanner.py` batch. It should contain durable per-target statuses from novel TESS targets after TOI/CTOI/confirmed-host exclusion. | [HUMAN] run the B5 command in `docs/DISCOVERY_RUNBOOK.md`, then paste the rank/filter summary back for review |
| `logs/discovery_filtered_001.json` | **PENDING** — generated only if triage finds candidates | Ignored | Expected filtered candidate list from `Skills/alert_filter.py logs/discovery_run_001.json --fpp-max 0.15`; an empty/nonexistent file after a no-match exit is acceptable if the full scan log exists. | Review alongside `logs/discovery_run_001.json`; update this ledger with candidate count or null result |
