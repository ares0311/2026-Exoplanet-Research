...
| `data/new_tess_targets.txt` + `data/new_tess_targets.json` | Pending Path A inventory | Ignored | Approved 2026-06-18 as the next T1-1 strategy; generated by `Skills/fetch_additional_tess_labels.py --corpus data/tess_snippets_v2.jsonl --output data/new_tess_targets.txt` | Human runs live ExoFOP inventory; agent reviews count, label balance, and source mix before any long MAST fetch |
| `data/tess_snippets_expansion_v3.jsonl` | Pending Path A snippet expansion | Ignored | Not generated yet; target source will be `data/new_tess_targets.json` if inventory justifies a long fetch | Fetch only after agent inventory review; audit parseability, finite flux, label balance, and duplicate TICs before merging |
| `data/tess_snippets_v3.jsonl` | Pending expanded TESS corpus | Ignored | Not generated yet; intended merge of `data/tess_snippets_v2.jsonl` plus `data/tess_snippets_expansion_v3.jsonl` | Build only after expansion audit passes; then rebuild validated v3 splits |
| `data/tess_cnn_splits_v3/` | Pending expanded TESS split | Ignored | Not generated yet; candidate-12 training is blocked until this split validates | Run `Skills/cnn_split_validator.py`; train only if PASS and artifact ledger is updated |
