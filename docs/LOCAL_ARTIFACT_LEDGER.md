...
| `logs/discovery_filtered_001.json` | **COMPLETE / EMPTY** | Ignored | 2026-06-28 local filter output from run 001: empty JSON list (`[]`), SHA-256 `4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945`; empty because run 001 produced no candidates | Retain locally only; next actionable triage output is `logs/discovery_filtered_002_qlp.json` |
| `logs/discovery_run_002_qlp.json` | **PENDING** — first QLP discovery scan not yet run | Ignored | Expected output from the B5 QLP 200-target `star_scanner.py` batch. It must contain durable per-target statuses from novel TESS targets after TOI/CTOI/confirmed-host exclusion and should not repeat the run 001 mostly no-data failure mode. | [HUMAN] run the QLP B5 command in `docs/DISCOVERY_RUNBOOK.md`, then paste the rank/filter summary back for review |
| `logs/discovery_filtered_002_qlp.json` | **PENDING** — generated after QLP triage | Ignored | Expected filtered candidate list from `Skills/alert_filter.py logs/discovery_run_002_qlp.json --fpp-max 0.15`; an empty file is acceptable only if the QLP scan contains real clear scans or candidates rather than mostly no-data statuses. | Review alongside `logs/discovery_run_002_qlp.json`; update this ledger with candidate count or meaningful null result |
