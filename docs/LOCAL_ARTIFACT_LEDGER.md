...
| `data/tess_combined_snippets.jsonl` | **NOT BUILT** — authorized to build | Ignored | Target ≈7,483 rows (TESS v2 2,619 + overlap 4,864); build command: `cat data/tess_snippets_v2.jsonl data/tess_kepler_overlap_snippets.jsonl > data/tess_combined_snippets.jsonl` | Build now; verify row count matches sum of sources; then run `build_cnn_training_data.py` |
| `data/tess_combined_cnn_splits/` | **NOT BUILT** — authorized to build | Ignored | Target ≈5,200 train / ≈1,100 val / ≈1,100 test; build command: `.venv/bin/python Skills/build_cnn_training_data.py data/tess_combined_snippets.jsonl --output-dir data/tess_combined_cnn_splits --seed 7` | Build after combined corpus is ready; validate with `cnn_split_validator.py`; train C13 after PASS |
| `checkpoints/cnn_tess_c13/` | **NOT TRAINED** — authorized | Ignored | Planned: train from `data/tess_combined_cnn_splits` with `checkpoints/cnn_kepler_pretrain/best.pt` as pretrained init; train command: `caffeinate -dims .venv/bin/python Skills/train_cnn.py --split-dir data/tess_combined_cnn_splits --checkpoint-dir checkpoints/cnn_tess_c13 --pretrained-checkpoint checkpoints/cnn_kepler_pretrain/best.pt --device auto` | Train after split validator PASS; evaluate with `evaluate_cnn_checkpoint.py`; promote only after evaluator PASS plus explicit human approval |
