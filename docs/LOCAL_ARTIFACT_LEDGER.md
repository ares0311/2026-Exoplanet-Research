...
| `data/tess_kepler_overlap_snippets.jsonl` | Partial / resume next | Ignored | User-reported 2026-06-19 partial run has 81 rows after a crash at progress printing. Root cause: the first concurrent fetcher still used Lightkurve `download_all()`, whose `suppress_stdout` decorator mutates process-global `sys.stdout` and is unsafe in worker threads. Corrected fetcher avoids `download_all()`, uses fail-soft progress output, groups pending KOIs by KIC, runs bounded rolling concurrency with default `--workers 4 --request-delay 0.25`, resumes successes, and skips terminal failures via sidecar | Resume Step 7c with the fixed script; do not delete partial rows unless JSONL/sidecar audit shows corruption; after completion audit row count, label balance, and sidecar before merging with TESS v2 |
| `data/tess_kepler_overlap_snippets.jsonl.failures.jsonl` | Present local sidecar | Ignored | User-reported 2026-06-19 overlap resume skipped 24 terminal failures from this sidecar; ordinary overlap reruns skip terminal failures recorded here; `--retry-failures` is required for intentional rechecks | Review with the overlap corpus after Step 7c before building combined splits |
| `data/tess_combined_snippets.jsonl` | Not generated / conditional | Ignored | Build only after the overlap corpus audit approves merge with `data/tess_snippets_v2.jsonl` | If approved, build from TESS v2 plus overlap corpus and immediately validate downstream splits |
| `data/tess_combined_cnn_splits/` | Not generated / conditional | Ignored | Build only after `data/tess_combined_snippets.jsonl` exists and passes corpus audit | Validate with `Skills/cnn_split_validator.py`; train C13 only after validator `PASS` and ledger update |
| `checkpoints/cnn_tess_c13/` | Not generated / conditional | Ignored | Candidate 13 depends on a validated combined TESS+overlap split; not authorized until C12 is rejected and overlap corpus audit passes | Train with `configs/cnn_tess_finetune_c12.json`, evaluate with production gates, and promote only after evaluator PASS plus explicit human approval |
