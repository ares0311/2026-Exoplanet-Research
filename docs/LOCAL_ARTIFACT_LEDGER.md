...
| `data/tess_cnn_splits/` | Pending | Ignored | Must be generated from `data/tess_snippets_v2.jsonl` for fine-tuning/evaluation | Build next; require `cnn_split_validator` PASS before fine-tuning |
...
| `checkpoints/cnn_tess_finetuned/` | Pending | Ignored | No approved checkpoint | Fine-tune from the MPS Kepler pretrain checkpoint after TESS split validation; run production gate evaluator |
