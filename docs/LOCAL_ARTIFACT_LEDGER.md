...
| `logs/discovery_filtered_002_qlp.json` | **COMPLETE / EMPTY / NON-CLOSING** | Ignored | 2026-06-28 local filter output from run 002: empty JSON list (`[]`), SHA-256 `4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945`; empty because run 002 produced no candidates and no clear scans | Retain locally only; next actionable triage output is `logs/discovery_filtered_003_qlp_cache_repair.json` |
| `logs/discovery_run_003_qlp_cache_repair.json` | **PENDING** — first QLP discovery scan after cache repair | Ignored | Expected output from the B5 QLP 200-target `star_scanner.py` batch after the shared Lightkurve fetcher repairs corrupt cache files by deleting only named `.fits` files inside `.lightkurve/.../mastDownload/...` and retrying. It must contain durable per-target statuses from novel TESS targets after TOI/CTOI/confirmed-host exclusion. | [HUMAN] after the cache-repair fix is merged, run the QLP B5 command in `docs/DISCOVERY_RUNBOOK.md`, then paste the rank/filter summary back for review |
| `logs/discovery_filtered_003_qlp_cache_repair.json` | **PENDING** — generated after cache-repair QLP triage | Ignored | Expected filtered candidate list from `Skills/alert_filter.py logs/discovery_run_003_qlp_cache_repair.json --fpp-max 0.15`; an empty file is acceptable only if the QLP scan contains real clear scans or candidates rather than mostly no-data/cache-error statuses. | Review alongside `logs/discovery_run_003_qlp_cache_repair.json`; update this ledger with candidate count or meaningful null result |
