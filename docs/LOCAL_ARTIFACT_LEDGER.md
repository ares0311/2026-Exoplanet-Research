...
| `checkpoints/cnn_tess_c12/` | Pending local run | Ignored | Not generated as of 2026-06-18; candidate 12 uses existing validated TESS splits, Kepler pretrain SHA `c782d7af61171b3f58447f7a49343c86618c447292a71bd28d540807835787c7`, and `configs/cnn_tess_finetune_c12.json` | Run `docs/CNN_PRODUCTION_RUNBOOK.md` Step 7b on the Mac, paste final training result and SHA, then evaluate only if validation AUC supports continuing |
| `data/tess_kepler_overlap_snippets.jsonl` | Not generated / conditional | Ignored | Candidate overlap corpus is script-ready but must not be fetched unless C12 is rejected; fetcher resumes successes and skips terminal failures via sidecar | If C12 misses the gate, run Step 7c once with `caffeinate`, then audit row count, label balance, and sidecar before merging with TESS v2 |
| `data/tess_kepler_overlap_snippets.jsonl.failures.jsonl` | Expected sidecar for conditional overlap fetch | Ignored | Not generated as of 2026-06-18; ordinary overlap reruns skip terminal failures recorded here; `--retry-failures` is required for intentional rechecks | Review with the overlap corpus after Step 7c before building combined splits |
