...
| `checkpoints/cnn_tess_c19/` | **REJECTED** — SHA-256 `65f3721fac577807f35e4edaeaa9cc0cd0f50959441344487f7c77f35a570436`; best epoch 29; val AUC 0.8218; early stop epoch 54; test raw AUC=0.8420, F1=0.7951, Brier=0.1606, ECE=0.0377; T=1.8785927; calibrated Brier=0.1658, ECE=0.0760; Flag: FAIL; regressed from C18 across all metrics; freeze_conv strategy exhausted | Ignored | 2026-06-22: trained from `data/tess_combined_cnn_splits` with Kepler pretrain; `configs/cnn_tess_c19.json` (freeze_conv_epochs=20); root cause: LR scheduler does not fire during frozen phase when val_auc is improving, so conv unfreezes at same LR as C18; longer frozen phase over-adapted FC head (T=1.88 vs T=1.61 in C18); no further freeze_conv variant expected to break 0.8439 ceiling | Do not promote or retry freeze_conv; next step is K2 EPIC overlap corpus (runbook Step 7g) |
| `data/tess_k2_overlap_snippets.jsonl` | **NOT YET BUILT** — fetcher committed 2026-06-22 | Ignored | K2 EPIC KOI confirmed planets/FPs with TESS re-observations folded at K2 ephemerides; analogous to `data/tess_kepler_overlap_snippets.jsonl`; expected to yield 500–1,500 new labeled TESS-domain snippets; build script: `Skills/fetch_tess_k2_overlap_snippets.py` (committed); failures sidecar: `data/tess_k2_overlap_snippets.jsonl.failures.jsonl` | [HUMAN] run multi-hour MAST fetch: `caffeinate -dims .venv/bin/python Skills/fetch_tess_k2_overlap_snippets.py --output data/tess_k2_overlap_snippets.jsonl --workers 4 --request-delay 0.25` then paste line count back |
| `data/tess_c20_combined_snippets.jsonl` | **NOT YET BUILT** | Ignored | C20 training corpus: TESS v2 + Kepler overlap + K2 overlap; assemble after K2 fetch is audited | [HUMAN] after K2 audit: `cat data/tess_snippets_v2.jsonl data/tess_kepler_overlap_snippets.jsonl data/tess_k2_overlap_snippets.jsonl > data/tess_c20_combined_snippets.jsonl` |
