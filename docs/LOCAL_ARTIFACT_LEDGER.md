...
| `data/tess_kepler_overlap_snippets.jsonl` | Not generated / next authorized fetch | Ignored | Candidate overlap corpus is script-ready; C12 is rejected; fetcher groups pending KOIs by KIC, runs bounded rolling concurrency with default `--workers 4 --request-delay 0.25`, resumes successes, and skips terminal failures via sidecar | Run Step 7c once with `caffeinate`, then audit row count, label balance, and sidecar before merging with TESS v2 |
