...
| `checkpoints/cnn_tess_c19/` | **NOT YET TRAINED** — `configs/cnn_tess_c19.json` committed | Ignored | Planned: train from `data/tess_combined_cnn_splits` with Kepler pretrain; `configs/cnn_tess_c19.json` (LR=1e-4, weight_decay=1e-3, use_batch_norm=false, augment=true, patience=25, freeze_conv_epochs=20); hypothesis: doubling the frozen period from C18 gives FC head more time to establish stable TESS-domain baseline and LR scheduler has more epochs to decay before conv unfreezes — should reduce val overconfidence (lower T) and improve AUC beyond 0.8439; train command: `caffeinate -dims .venv/bin/python Skills/train_cnn.py --split-dir data/tess_combined_cnn_splits --checkpoint-dir checkpoints/cnn_tess_c19 --config configs/cnn_tess_c19.json --pretrained-checkpoint checkpoints/cnn_kepler_pretrain/best.pt --device auto` | [HUMAN] train and evaluate with `evaluate_cnn_checkpoint.py`; promote only after evaluator PASS plus explicit human approval |
