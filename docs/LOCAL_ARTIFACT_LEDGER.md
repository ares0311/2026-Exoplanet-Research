...
| `data/joint_cnn_splits/` | **NOT YET BUILT** | Ignored | Planned for C17: merge Kepler train (4,741) + TESS combined train (4,892) = ~9,633 joint training examples; TESS-only val (1,049) and test (1,033) unchanged; hypothesis: 4,892-example data ceiling drove 0.83–0.84 AUC; 2× more examples should push above 0.85 | [HUMAN] `git pull origin main` then `.venv/bin/python Skills/build_joint_cnn_splits.py` then validate with `.venv/bin/python Skills/cnn_split_validator.py data/joint_cnn_splits` |
| `checkpoints/cnn_tess_c17/` | **NOT YET TRAINED** — `configs/cnn_tess_c17.json` committed; joint splits needed first | Ignored | Planned: train from `data/joint_cnn_splits` with Kepler pretrain; `configs/cnn_tess_c17.json` (LR=1e-4, weight_decay=1e-3, use_batch_norm=false, augment=true, patience=30, full-unfreeze from epoch 1); train command: `caffeinate -dims .venv/bin/python Skills/train_cnn.py --split-dir data/joint_cnn_splits --checkpoint-dir checkpoints/cnn_tess_c17 --config configs/cnn_tess_c17.json --pretrained-checkpoint checkpoints/cnn_kepler_pretrain/best.pt --device auto` | [HUMAN] trains after joint splits validated; evaluate with `evaluate_cnn_checkpoint.py`; promote only after evaluator PASS plus explicit human approval |
