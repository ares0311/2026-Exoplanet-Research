...
| K2 overlap corpus (`data/tess_k2_overlap_snippets.jsonl`) | **INVALID — NEEDS RE-FETCH** — 2,086 snippets (2026-06-27), phase-folded at a corrupted epoch (BJD/BKJD double-offset bug, fixed 2026-07-10); not used in any promoted model |
| Dataset/model-training handoff brief | **ACTIVE CONTRACT** — `docs/exoplanet_exomoon_dataset_handoff.md` |

**T1-1 promoted on 2026-07-09:** the master-corpus Kepler CNN checkpoint passed the held-out gates (AUC 0.9572, calibrated F1 0.8347), received explicit human approval, and is promoted as the frozen `benchmark_cnn_v1` artifact under `models/cnn/benchmark_cnn_v1/`. Do not restart old CNN experiments unless a named validation gate fails.

**T1-2 COMPLETE (2026-07-10) — NO TIER 1 GAPS REMAIN OPEN.** The held-out K2 calibration set was built, native snippets fetched live (4-way sharded), CNN predictions merged, and stacking weights calibrated: **XGBoost=0.95 / CNN=0.00 / Bayesian=0.05** (AUC 0.9576), now wired into `cli.py`'s `full-ensemble` blend and `StackingScorer`'s defaults. A severe bug was found and fixed in the process: `CnnScorer` silently returned a flat, fabricated `cnn_prob=0.5` for every prediction when invoked via the standard `python Skills/foo.py` pattern, because the repo root wasn't guaranteed on `sys.path` for its `Skills.*` absolute imports — no error, no warning, and the bogus run happened to produce the *same* numeric weight as the corrected run, which is what made it dangerous. Fixed with `_ensure_repo_root_on_sys_path()` plus a `RuntimeWarning` on genuine load failures (see `docs/PRODUCTION_READINESS.md` T1-2 for the full incident record and `src/exo_toolkit/ml/cnn_scorer.py`). Per `docs/PRODUCTION_READINESS.md`'s Live-Readiness Summary, every scorer mode is now `PRODUCTION READY` or `PROMOTED`, and Tier 2 is closed (T2-1 complete, T2-2/T2-3 permanently out of scope). **Before starting any new work, re-check `docs/PRODUCTION_READINESS.md` fresh for the current gap list — do not assume this file's dated history below is current.**
...
9. **[HUMAN COMPLETE] Live K2 snippet fetch (2026-07-10):** ran sharded 4-way (`--shard-count 4`, `--workers 6` each) — 596/596 targets processed, 588 snippets written, 8 terminal failures, ~3.5 min combined wall-clock. Concatenated to `data/t1_2_k2_calibration_snippets.jsonl` (local-only, gitignored).
10. **[AGENT COMPLETE] Stacking calibration closed out (2026-07-10):** `Skills/merge_t1_2_k2_cnn_predictions.py` scored the 588 snippets with `benchmark_cnn_v1` (explicit cross-mission use — see `data_selection/data_selection_decision_log.md`); found and fixed the severe `CnnScorer` sys.path bug described above; `Skills/calibrate_stacking_weights.py` produced `models/stacking_weights.json` (XGBoost=0.95/CNN=0.00/Bayesian=0.05, AUC 0.9576); `StackingScorer` and `cli.py`'s `full-ensemble` defaults updated to match. Merged via PR #196. **T1-2 is complete — see the note above item 1.**
...
| K2 overlap corpus (`data/tess_k2_overlap_snippets.jsonl`) | **INVALID — NEEDS RE-FETCH** — 2,086 snippets written 2026-06-27 (wrote=2086, skipped=174, terminal_failures=135, elapsed=2531s), but phase-folded at a corrupted epoch (BJD/BKJD double-offset bug, fixed 2026-07-10); not used in any promoted model |
| T1-2 K2 calibration corpus (`data/t1_2_k2_calibration_snippets.jsonl`) | **COMPLETE** — 588 native K2 snippets (2026-07-10, 4-way sharded fetch); distinct from the invalid row above — this is native K2 flux, not K2-ephemeris-folded TESS flux. See `docs/LOCAL_ARTIFACT_LEDGER.md` |
