...
**T1-1 promoted on 2026-07-09:** the master-corpus Kepler CNN checkpoint passed the held-out gates (AUC 0.9572, calibrated F1 0.8347), received explicit human approval, and is promoted as the frozen `benchmark_cnn_v1` artifact under `models/cnn/benchmark_cnn_v1/`. Do not restart old CNN experiments unless a named validation gate fails. The next Tier 1 gap is T1-2 stacking calibration after the promotion PR is merged and CI-clean.
...
7. **[AGENT NEXT] Post-T1-1 work:** after the promotion PR is merged and CI-clean, start T1-2 stacking calibration on a held-out calibration set; do not tune full-ensemble weights against training or frozen-eval data.
