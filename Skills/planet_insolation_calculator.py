...
def compute_insolation(
    ...
    HZ boundaries scaled from Kopparapu et al. (2013): inner ~0.97 AU, outer ~1.67 AU,
    scaled as L^0.5.
    """
    if not math.isfinite(luminosity_lsun) or luminosity_lsun <= 0:
        return InsolationResult(
            ...
            insolation_w_m2=float("nan"), insolation_searth=float("nan"),
            equilibrium_temp_k=float("nan"), hz_inner_au=float("nan"),
            hz_outer_au=float("nan"), in_hz=False, flag="INVALID_LUMINOSITY",
        )
    if not math.isfinite(semi_major_axis_au) or semi_major_axis_au <= 0:
        return InsolationResult(
            ...
            insolation_w_m2=float("nan"), insolation_searth=float("nan"),
            equilibrium_temp_k=float("nan"), hz_inner_au=float("nan"),
            hz_outer_au=float("nan"), in_hz=False, flag="INVALID_SEMI_MAJOR_AXIS",
        )
    if not math.isfinite(albedo) or albedo < 0 or albedo >= 1.0:
        return InsolationResult(
            luminosity_lsun=luminosity_lsun, semi_major_axis_au=semi_major_axis_au,
            insolation_w_m2=float("nan"), insolation_searth=float("nan"),
            equilibrium_temp_k=float("nan"), hz_inner_au=float("nan"),
            hz_outer_au=float("nan"), in_hz=False, flag="INVALID_ALBEDO",
        )

    lum_w = luminosity_lsun * _LSUN_W
    a_m = semi_major_axis_au * _AU_M
    flux_w_m2 = lum_w / (4.0 * math.pi * a_m**2)
    flux_searth = flux_w_m2 / _SFLUX_EARTH
    ...
    return InsolationResult(
        ...
        insolation_searth=round(flux_searth, 4),
...
def _cli() -> int:
    ...
    args = p.parse_args()
    r = compute_insolation(args.luminosity_lsun, args.semi_major_axis_au, args.albedo)
