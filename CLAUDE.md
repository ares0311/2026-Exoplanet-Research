...
- `exo --version` / `exo -V` — prints the installed `exo-toolkit` package version (currently `0.2.9`)
- fallback version `0.2.9` in `src/exo_toolkit/__init__.py` is used only if source-tree and installed package metadata are unavailable
