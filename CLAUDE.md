...
2. If the target script does not yet support the applicable form of parallelism, that absence is itself worth surfacing: either add it (mirroring the sharding/`--workers` pattern already proven in `process_t1_kepler_batch.py`) before recommending a multi-hour sequential recipe, or explicitly tell the user why not (e.g. a one-off task not worth the engineering cost).
   - **Shard-capable scripts today** (check this list before deciding whether to add sharding or reuse it): `Skills/process_t1_kepler_batch.py` (`--shard-index`/`--shard-count`, version 0.2.18) and `Skills/fetch_t1_2_k2_calibration_snippets.py` (added when scoping the T1-2 live fetch — partitions by `epic_id % shard_count`, auto-suffixed per-shard output/failure files). Any other acquisition/processing script (see the Run Report Policy retrofit list above) remains sequential-per-process/`--workers`-only until a live recipe for it is expected to run long enough to justify adding the same pattern.
...
**K2 corpus fetch status (as of 2026-06-27):** 2,086 snippets were written to `data/tess_k2_overlap_snippets.jsonl` (wrote=2086, skipped=174, terminal_failures=135, elapsed=2531s / 42m11s). **UPDATE 2026-07-10: this file is INVALID and must be re-fetched before any future use.** `Skills/fetch_tess_k2_overlap_snippets.py` assumed k2pandc's `pl_tranmid` was BKJD and added the 2454833-day offset a second time; live verification while building the unrelated T1-2 calibration manifest confirmed `pl_tranmid` is already full BJD, so every one of these 2,086 snippets was phase-folded at a corrupted epoch. The bug is fixed in the script (see `docs/PRODUCTION_READINESS.md` T1-2 and `data_selection/data_selection_decision_log.md`'s 2026-07-10 entry); no promoted model was trained on the corrupted data (it predates any C20-style TESS combined-corpus assembly), but re-running the fetch is required before this file can be trusted again.
...
7. **[AGENT COMPLETE, 2026-07-10]** T1-2 held-out calibration manifest (596 rows from `k2pandc`, disjoint EPIC namespace from the Kepler KIC-based training/val/frozen-eval splits) plus catalog-only Bayesian+XGBoost scoring are built and committed — `Skills/build_t1_2_k2_calibration_manifest.py`, `Skills/score_t1_2_k2_calibration.py`. Two real unit bugs were found and fixed while building this (see `docs/PRODUCTION_READINESS.md` T1-2): `pl_tranmid` is already full BJD (not BKJD), and `pl_trandur` is empirically in hours despite the archive's own schema claiming days.
8. **[AGENT COMPLETE, 2026-07-10]** Native K2 light-curve snippet fetcher built and offline-tested — `Skills/fetch_t1_2_k2_calibration_snippets.py` (production `fetch_lightcurve()` path, resume, terminal-failure logging, bounded `--workers`, Run Report Policy). Not yet run live.
9. **[HUMAN NEXT]** Run `caffeinate -dims .venv/bin/python Skills/fetch_t1_2_k2_calibration_snippets.py --manifest metadata/t1_2_k2_calibration_manifest.jsonl --output data/t1_2_k2_calibration_snippets.jsonl --workers 4` (~600 targets).
10. **[AGENT NEXT]** After the live fetch, merge `cnn_prob` from `benchmark_cnn_v1` into the partial predictions, run `Skills/calibrate_stacking_weights.py`, and update `StackingScorer`'s default weights per its printed recipe.
...
- **K2 overlap**: `data/tess_k2_overlap_snippets.jsonl` — **INVALID, NEEDS RE-FETCH** — 2,086 snippets written 2026-06-27 (wrote=2086, skipped=174, terminal_failures=135, elapsed=2531s), but phase-folded at a corrupted epoch (double-counted BJD offset, fixed 2026-07-10); not used in any promoted model
...
**Cross-mission CNN scoring guard (added when promoting the Kepler checkpoint):** every trained checkpoint should declare which mission it was trained on via `train_cnn.py --mission TESS|Kepler|K2|JWST`, stamped into the saved `config.json`'s `training_mission` field. `run_pipeline()`/`exo scan` **refuses by default** to apply a CNN checkpoint whose declared (or undeclared) mission doesn't match the scan's `--mission`, since Kepler↔TESS CNN transfer has repeatedly failed this project's production gates even after deliberate fine-tuning (see the C11-C19 history above). The override is `allow_cross_mission_cnn=True` / `--allow-cross-mission-cnn`, intended only for deliberate out-of-domain testing, not routine use. Checkpoints trained before this field existed have `training_mission=None` and are blocked by the same guard unless explicitly patched or the override is passed.

The accepted `train_cnn.py` flags are `--split-dir`, `--checkpoint-dir`, `--pretrained-checkpoint`, and `--mission`.
