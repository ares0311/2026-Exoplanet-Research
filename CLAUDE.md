...
- `exo --version` / `exo -V` — prints the installed `exo-toolkit` package version (currently `0.2.8`)
- fallback version `0.2.8` in `src/exo_toolkit/__init__.py` is used only if source-tree and installed package metadata are unavailable
- Each output row gains a `"features"` dict plus a `"meta"` dict: `toolkit_version`, `run_at`, `scorer`, `git_commit`, `features_available`
