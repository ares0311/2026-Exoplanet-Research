...
Use `docs/CNN_PRODUCTION_RUNBOOK.md` for authoritative copy-paste workflow. **Step C is complete: `checkpoints/cnn_t1_1_kepler/best.pt` passed the production gates** (raw test AUC 0.9252, calibrated F1 0.8281, temperature T=1.0 — no calibration degradation) — the first checkpoint in the full C1-C19 + T1-1 history to do so. Promotion to `models/` requires explicit human approval, not yet granted. Separately, a Kepler corpus expansion source was found and verified live: `Q1_Q17_DR24_TCE` (NASA Exoplanet Archive TAP) has real `av_training_set` labels for 7,091 genuinely new KIC targets beyond the current manifest. `Skills/build_t1_dr24_tce_expansion_manifest.py` (version 0.2.26) is committed and tested; next `[HUMAN]` action is to run it live (catalog-only, no FITS download) — see `docs/CNN_PRODUCTION_RUNBOOK.md`'s "Kepler Corpus Expansion" section.
