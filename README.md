...
| 🟡 | **Production CNN checkpoint** | C1-C19 were rejected; source verification, storage estimates, immutable source snapshots, and the leakage-safe Kepler manifest are committed. The first bounded Kepler processing batch live-smoked successfully: 25 targets, 26 snippets, 0 failed rows. Remaining work is continued bounded processing, split validation, training, evaluation, and promotion only after held-out gates pass. | `metadata/t1_1_kepler_training_manifest.jsonl`, `metadata/t1_1_kepler_manifest_summary.json`, `docs/CNN_SPEC.md`, `docs/CNN_PRODUCTION_RUNBOOK.md`, `docs/exoplanet_exomoon_dataset_handoff.md`, `docs/PRODUCTION_READINESS.md` |
